...
test("global account capacity prioritizes Campaign B Email 3 over Campaign A Email 1", () => {
  const email1 = candidates(1, 25, { prefix: "campaign-a" }).map((row) => ({
    ...row,
    campaignId: "campaign-a",
  }));
  const email3 = candidates(3, 10, { prefix: "campaign-b" }).map((row) => ({
    ...row,
    campaignId: "campaign-b",
  }));
  const result = applyGlobalSchedulingPolicy([...email1, ...email3], {
    accountDailyLimit: 25,
    campaignLimits: new Map([
      ["campaign-a", { totalDailyLimit: 25, newContactsPerDay: 25 }],
      ["campaign-b", { totalDailyLimit: 25, newContactsPerDay: 25 }],
    ]),
  });
  const scheduled = result.outcomes.filter((row) => row.action === "scheduled");

  assert.equal(scheduled.length, 25);
  assert.equal(scheduled.filter((row) => row.current_step === 3).length, 10);
  assert.equal(scheduled.filter((row) => row.current_step === 1).length, 15);
});

test("global policy enforces account, campaign, existing, and broker capacity together", () => {
  const rows = [
    ...candidates(3, 4, {
      prefix: "campaign-a",
      domain: "broker.example",
      domainLimit: 2,
    }).map((row) => ({ ...row, campaignId: "campaign-a" })),
    ...candidates(2, 5, { prefix: "campaign-b" }).map((row) => ({
      ...row,
      campaignId: "campaign-b",
    })),
  ];
  const result = applyGlobalSchedulingPolicy(rows, {
    accountDailyLimit: 6,
    campaignLimits: new Map([
      ["campaign-a", { totalDailyLimit: 4, newContactsPerDay: 4 }],
      ["campaign-b", { totalDailyLimit: 4, newContactsPerDay: 4 }],
    ]),
    existingAccountScheduled: 2,
    existingCampaignScheduled: new Map([["campaign-a", 1], ["campaign-b", 1]]),
    brokerDomainCounts: new Map([["broker.example", 1]]),
  });

  assert.equal(result.outcomes.filter((row) => row.action === "scheduled").length, 4);
  assert.equal(
    result.outcomes.filter(
  ...
  assert.equal(
    result.outcomes.filter(
      (row) => row.safetyStatus === "account_limit_reached",
    ).length,
    2,
  );
});
...
test("production persistence can batch schedule and enrollment writes", async () => {
  const policy = runPolicy([
    ...candidates(3, 1),
    ...candidates(2, 2, {
      prefix: "limited",
    ...
    ...candidates(1, 1, {
      restriction: {
        kind: "stop",
        reason: "Suppressed.",
        safetyStatus: "suppressed",
      },
    }),
  ]);
  const calls = { schedules: [], rollForwards: [], stops: [] };

  await persistScheduleOutcomes(policy.outcomes, {
    writeSchedule: async () => assert.fail("single schedule write used"),
    rollForwardEnrollment: async () => assert.fail("single roll-forward used"),
    stopEnrollment: async () => assert.fail("single stop used"),
    writeSchedules: async (outcomes) => calls.schedules.push(...outcomes),
    rollForwardEnrollments: async (outcomes) => calls.rollForwards.push(...outcomes),
    stopEnrollments: async (outcomes) => calls.stops.push(...outcomes),
  });

  assert.equal(calls.schedules.length, policy.outcomes.length);
  assert.ok(calls.rollForwards.every((outcome) => outcome.action === "roll_forward"));
  assert.ok(calls.stops.every((outcome) => outcome.action === "stop"));
  assert.equal(calls.rollForwards.length, 1);

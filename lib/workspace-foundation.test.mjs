...
test("B2 relationships require clients and campaigns in the same workspace", () => {
  for (const constraint of [
    "client_events_workspace_client_fk",
    "email_templates_workspace_campaign_fk",
    "campaign_steps_workspace_campaign_fk",
  ]) {
    assert.match(sql, new RegExp(`constraint ${constraint} foreign key \\(workspace_id,`));
  }
});
...
test("clients explicitly enables RLS without restoring browser access or a permissive policy", () => {
  assert.match(sql, /alter table public\.clients enable row level security/);
  assert.match(sql, /revoke all privileges on table[\s\S]*public\.clients[\s\S]*from anon, authenticated/);
  assert.match(sql, /revoke all privileges on table public\.clients from service_role/);
  assert.match(sql, /grant select, insert, update, delete on table public\.clients to service_role/);
  assert.doesNotMatch(sql, /create policy[\s\S]{0,200}on public\.clients/i);
});

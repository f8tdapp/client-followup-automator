...
test("Home displays one enabled draft action for existing schedule availability", () => {
  for (const dailySendPlan of [plan(2), plan(0, [{ status: "scheduled" }])]) {
    const html = renderHome({ ...connectedHome, dailySendPlan });
    const buttons = html.match(new RegExp(draftButton.source, "g")) ?? [];
    const button = buttons[0];
    assert.ok(button);
    assert.equal(buttons.length, 1);
    assert.doesNotMatch(button, / disabled=/);
    assert.doesNotMatch(html, /Go to Today&#x27;s Follow-Ups/);
  }
});
...
test("draft generation retains its in-progress disabled state", () => {
  const html = renderHome({
    ...connectedHome,
    dailySendPlan: plan(1),
    isGeneratingDrafts: true,
  });
  assert.match(html, /<button\b[^>]*disabled=""[^>]*>Working\.\.\.<\/button>/);
});

...
test("recommendations identify the constrained campaign", () => {
  ...
  assert.match(
    ...
    /Pause new enrolments for Real Estate Agent Follow-Up\. Its active queue has \d+ projected backlog items after 30 days\./,
  );
});
...
test("production forecast dependency and GET refresh remain read-only", async () => {
  ...
  const GET = createWorkloadForecastGetHandler(load, async () => ({
    ok: true,
    user: { id: "owner-test" },
  }));
